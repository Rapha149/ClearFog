...
public class Wrapper1_19_R2 implements VersionWrapper {
    ...
    public List<ChannelPipeline> getServerPipelines() throws NoSuchFieldException, IllegalAccessException {
        ServerConnection conn = ((CraftServer) Bukkit.getServer()).getServer().ac();
    ...
    public void updateViewDistance(Player player, int viewDistance, boolean directUpdate) {
        ...
        p.b.a(new PacketPlayOutViewDistance(viewDistance));

        if (directUpdate) {
            PlayerChunkMap map = p.y().k().a;
